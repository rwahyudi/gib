...
`ib dns search KEY` searches only the resolved current zone by default. If KEY is a complete forward FQDN and no explicit `--zone` or `--global` scope is supplied, search infers the longest matching authoritative zone from the selected DNS view, searches that zone, and also matches the relative owner name. Use `-z ZONE` to choose a different root zone, and add `-r` or `--recursive` to include child authoritative zones under that root. `--global` still searches every searchable zone in the selected DNS view and cannot be combined with `--recursive`. Search supports the same `-t`/`--type`, `-e`/`--exclude`, `-s`/`--sort`, and `-C`/`--columns` record filters as list.
