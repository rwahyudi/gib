...
When an older Infoblox WAPI rejects `assigned_vlan` or `assigned_vlan_name` in network/networkcontainer `_return_fields`, `ib` retries that object query with base network fields and leaves the assigned VLAN columns empty instead of failing the list/search command.
