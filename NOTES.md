...
The `Sync Azure DevOps` workflow mirrors GitHub `main` and `v*` tags to Azure DevOps after pushes. It uses the `AZURE_DEVOPS_UNIX_GIB_PAT` secret through non-interactive Git credential prompts and defaults to `https://rwahyudi@dev.azure.com/rwahyudi/unix/_git/gib`; set repository variable `AZURE_DEVOPS_UNIX_GIB_URL` to override the target URL without changing the workflow. The PAT must have write access to the Azure DevOps Git repo.
