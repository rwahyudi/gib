...
func TestDNSListFallsBackWhenAllRecordsRejectsCreationTime(t *testing.T) {
	...
	app, stdout := dnsWorkflowApp(t, server.URL, server.URL)
	if err := app.Execute([]string{"-o", "json", "dns", "list"}); err != nil {
		t.Fatalf("dns list fallback: %v\nstdout:\n%s", err, stdout.String())
	}
	if len(allRecordRequests) != 2 {
		t.Fatalf("allrecords requests = %#v, want retry without creation_time", allRecordRequests)
	}
	if !strings.Contains(allRecordRequests[0], "creation_time") {
		t.Fatalf("first allrecords request missing creation_time: %#v", allRecordRequests)
	}
	if strings.Contains(allRecordRequests[1], "creation_time") {
		t.Fatalf("fallback allrecords request still included creation_time: %#v", allRecordRequests)
	}
	var rows []map[string]any
	if err := json.Unmarshal([]byte(stdout.String()), &rows); err != nil {
		t.Fatalf("decode records JSON: %v\n%s", err, stdout.String())
	}
	if len(rows) != 1 || cleanString(rows[0]["created"]) != "" {
		t.Fatalf("rows = %#v, want one row with blank created", rows)
	}
}
...
func TestDNSListDetailsFallsBackWhenRecordDetailRejectsCreationTime(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(r.URL.Path, "/record:a/ref"):
			fields := r.URL.Query().Get("_return_fields")
			detailRequests = append(detailRequests, fields)
			if strings.Contains(fields, "creation_time") {
				w.WriteHeader(http.StatusBadRequest)
				_ = json.NewEncoder(w).Encode(map[string]string{"text": "Unknown argument/field: 'creation_time'"})
				return
	...
	if !strings.Contains(detailRequests[0], "creation_time") {
		t.Fatalf("first detail request missing creation_time: %#v", detailRequests)
	}
	if strings.Contains(detailRequests[1], "creation_time") {
		t.Fatalf("fallback detail request still included creation_time: %#v", detailRequests)
	}
	var rows []map[string]any
	if err := json.Unmarshal([]byte(stdout.String()), &rows); err != nil {
		t.Fatalf("decode records JSON: %v\n%s", err, stdout.String())
