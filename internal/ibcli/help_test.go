...
func TestConfigHelpCoversWorkflowAndStorage(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Profile Details",
		"server reachability/TLS trust, unauth/auth WAPI discovery, validated credentials, auto GCM read endpoint/TLS trust, DNS view, default zone, audit logging",
		"merges when present",
...
func TestCreateHelpUsesStyledUsagePanel(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ib dns create ns <child-zone> <nameserver>",
		"delegate address 255.255.255.255",
