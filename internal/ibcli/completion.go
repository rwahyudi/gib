...
func vlanSortFlagCompletion(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return vlanSortCompletions(toComplete), cobra.ShellCompDirectiveNoFileComp
}

func (a *App) completeVLANSortValue(args []string) bool {
	prefix, ok := vlanSortCompletionPrefix(args)
	if !ok {
		return false
	}
	rows := vlanSortCompletions(prefix)
	noDescriptions := args[0] == "__completeNoDesc"
	for _, row := range rows {
		if noDescriptions {
			row = strings.SplitN(row, "\t", 2)[0]
...
func vlanSortCompletionPrefix(args []string) (string, bool) {
	if len(args) < 4 || !strings.HasPrefix(args[0], "__complete") || !isVLANListOrSearchArgs(args) {
		return "", false
	}
	current := args[len(args)-1]
	if current == "--sort" || current == "-s" {
...
func vlanSortCompletions(toComplete string) []string {
	prefix := strings.ToLower(strings.TrimSpace(toComplete))
	var rows []string
	for _, field := range vlanSortFields {
		appendVLANSortCompletion(&rows, field, "ascending", prefix)
		appendVLANSortCompletion(&rows, "-"+field, "descending", prefix)
	}
	return rows
}

func appendVLANSortCompletion(rows *[]string, value string, direction string, prefix string) {
	if prefix != "" && !strings.HasPrefix(value, prefix) {
		return
	}
	field := strings.TrimPrefix(value, "-")
	description := vlanSortDescriptions[field]
	if description == "" {
		*rows = append(*rows, value)
		return
	}
...
func vlanColumnFlagCompletion(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	return vlanColumnCompletions(toComplete), cobra.ShellCompDirectiveNoFileComp
}

func vlanColumnCompletions(toComplete string) []string {
	raw := strings.ToLower(strings.TrimSpace(toComplete))
	parts := strings.Split(raw, ",")
	prefix := parts[len(parts)-1]
	selected := map[string]bool{}
	...
	var rows []string
	for _, column := range vlanSelectableOutputColumns {
		if selected[column] {
			continue
		}
		if prefix != "" && !strings.HasPrefix(column, prefix) {
		...
		candidate := base + column
		description := vlanColumnDescriptions[column]
		if description == "" {
			rows = append(rows, candidate)
			continue
		}
...
func (a *App) vlanArgCompletion(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	if len(args) > 0 || strings.HasPrefix(strings.TrimSpace(toComplete), "-") {
		return flagCompletions(cmd, toComplete), cobra.ShellCompDirectiveNoFileComp
	}
	return a.completeVLANIdentifiers(cmd, toComplete), cobra.ShellCompDirectiveNoFileComp
}

func (a *App) completeVLANIdentifiers(cmd *cobra.Command, toComplete string) []string {
	profile, err := a.loadConfig(true)
	if err != nil {
		return nil
	}
	networkView := commandCompletionFlagValue(cmd, "network-view")
	vlans, err := a.cachedVLANsForCompletion(profile, networkView)
	if err != nil {
		return nil
	}
	return matchingVLANIdentifiers(vlans, toComplete)
}

func (a *App) cachedVLANsForCompletion(profile Profile, networkView string) ([]map[string]any, error) {
	entry, err := a.readCachedVLANs(profile, networkView)
	if err != nil {
		return nil, err
	}
	...
	if !entry.CacheFound {
		if prefetchEnabled {
			a.startVLANCacheRefreshAsync(profile, networkView)
		}
		return nil, nil
	}
...
func matchingVLANIdentifiers(vlans []map[string]any, toComplete string) []string {
	prefix := strings.ToLower(strings.TrimSpace(toComplete))
