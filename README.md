...
| `ib dns create TYPE NAME VALUE [ADDRESS]` | Create a DNS record, for example `ib dns create host app 192.0.2.10 -c "Application host"`. For PTR, use `ib dns create ptr IP_ADDRESS PTR_TARGET`; the reverse zone is auto-detected unless `--zone` is supplied. For NS, use `ib dns create ns CHILD_ZONE NS_HOST [ADDRESS]` to create an Infoblox delegated zone; when the address is omitted, `ib` resolves the nameserver and errors locally if it does not resolve. |
