...
`ib config list` includes a `scope` column so local and global profiles are visible in one merged table. Table output highlights `global` scope cells with a red background; JSON and CSV remain plain. Current-context footers mark a selected global profile as `Profile: NAME (global)`.

Audit logging is disabled by default. When enabled during `ib config new` or `ib config edit`, only successful write actions are logged: DNS record create/edit/delete, DNS zone create/delete, PTR side effects from A/AAAA/host workflows, and config profile create/edit/delete. Read, list, search, cache, and completion operations are not logged. Linux can log to JSON Lines files or syslog; Windows can log to Windows Event Log or a JSON Lines file. File logging defaults to `~/.ib/audit.jsonl` for local profiles, `/etc/ib/audit.jsonl` for Linux global profiles, and the current config directory on Windows. The file option warns that writable logs are weaker audit evidence, lets you return to method selection, and tests that the chosen path can be opened for writing before saving. Audit failures print a warning but do not fail the completed write action. Secret-looking fields such as passwords, keys, credentials, tokens, and secrets are redacted.
...
DNS listing/search and IPAM read workflows prefer Badger cache rows from the selected local or global config scope. Freshness is calculated from `cached_at + cache_ttl`; stale rows inside `records_cache_swr_ttl` are returned immediately while cache freshness is renewed or refreshed. DNS records can renew from cached zone-list SOA serials before falling back to serial checks and `/allrecords`; stale multi-zone record revalidation is batched and dispatches zones with the highest cached record counts first so larger refreshes enter the worker pool early. IPAM cache refreshes skip serial checks and re-download the relevant WAPI object.
...
| `ib config list` | List configured profiles with local/global scope, username, WAPI, SSL, DNS context, and merged config metadata in table output. `global` scope cells are red-highlighted in table output only. |
...
| `ib dns delete TYPE NAME [ZONE]` | Delete a DNS record; A/AAAA/host deletes also remove related PTR records when present and refresh affected reverse-zone cache. Prompts for confirmation unless `-y` is used. |
...
| `ib net list [SEARCH]` | List IPv4 networks and containers. Add `--network-view` to filter by IPAM network view, `--refresh` to wait for fresh WAPI data, `-s/--sort FIELD` to sort by `network`, `type`, `network_view`, `assigned_vlan`, `assigned_vlan_name`, or `comment`, and `-C/--columns LIST` to print selected columns including `extattrs`. |
| `ib net search KEYWORD` | Search IPv4 networks and containers by type, CIDR, network view, assigned VLAN ID, assigned VLAN name, comment, or extensible attribute values. CIDR matches also include related parent and child networks or containers in the same network view; add `--refresh` to wait for fresh WAPI data. |
| `ib net show NETWORK` | Show details for one IPv4 network or container. Use `--network-view` when a CIDR exists in multiple network views. Extensible attributes are shown as individual field rows in table output and as a nested map in JSON output. |
| `ib net address IP` | Show IPAM details for an IPv4 address, including network, parent container, status, types, names, MAC address, lease state, comment, and extensible attributes when available. |
...
| `ib vlan list [SEARCH]` | List VLANs derived from the `vlans` array on IPAM networks and containers. Default columns are `vlan_id`, `name`, `parent`, `networks`, and `comment`. Add `--network-view` to filter by IPAM network view, `--refresh` to wait for fresh WAPI data, `-s/--sort FIELD` to sort by `vlan_id`, `name`, `parent`, `network_view`, `networks`, or `comment`, and `-C/--columns LIST` to print selected columns. |
| `ib vlan search KEYWORD` | Search VLANs by id, name, parent, network view, network, or comment. Add `--refresh` to wait for fresh WAPI data. |
...
`ib dns delete TYPE NAME` prompts before deleting. Use `-y` or `--yes` to skip the confirmation. Deleting an A/AAAA record also removes the matching PTR record in the discovered reverse zone when present; deleting a host record removes related PTR records for its addresses and refreshes affected reverse-zone cache. If multiple records of that type match, interactive table mode shows a Huh select list so one record can be chosen.
...
ib net list --columns network,type,extattrs -o json | jq '.[]'
```
...
fields are `network`, `type`, `network_view`, `assigned_vlan`, `assigned_vlan_name`, `comment`, and `extattrs`. Use `--columns` or `-C`
...
Multi-zone search preloads matching record-cache rows with one Badger handle before workers start and reuses zone-list SOA serials to renew unchanged stale record caches without per-zone HTTP checks. Stale rows that still need background revalidation are handed to one batch helper instead of one helper process per zone, with zones dispatched by descending cached record count so larger zones start first. Workers still fall back to per-zone cache/WAPI checks for missing, changed, or serial-less rows. The WAPI HTTP client keeps a larger per-host connection pool sized from `dns_search_worker_limit` so parallel search can reuse TLS connections instead of repeatedly reconnecting. When `dns_search_worker_limit` is greater than 10 and `read_server` is configured, `dns_search_primary_read_percent` (default `20`) routes that share of worker GETs back to the primary server.
