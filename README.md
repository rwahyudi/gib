...
`ib config list` includes a `scope` column so local and global profiles are visible in one merged table. Table output highlights global-scope rows with a red background; JSON and CSV remain plain. Current-context footers mark a selected global profile as `Profile: NAME (global)`.
...
| `ib config list` | List configured profiles with local/global scope, username, WAPI, SSL, DNS context, and merged config metadata in table output. Global-scope rows are red-highlighted in table output only. |
