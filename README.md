...
| `ib net list [SEARCH]` | List IPv4 networks and containers. Add `--network-view` to filter by IPAM network view, `--refresh` to wait for fresh WAPI data, `-s/--sort FIELD` to sort by `network`, `type`, `network_view`, `assigned_vlan`, `assigned_vlan_name`, or `comment`, and `-C/--columns LIST` to print selected columns. |
| `ib net search KEYWORD` | Search IPv4 networks and containers by type, CIDR, network view, assigned VLAN ID, assigned VLAN name, or comment. CIDR matches also include related parent and child networks or containers in the same network view; add `--refresh` to wait for fresh WAPI data. |
...
fields are `network`, `type`, `network_view`, `assigned_vlan`, `assigned_vlan_name`, and `comment`. Use `--columns` or `-C`
